...
# 读取文件
hp1_df = pd.read_csv(args.file_hp1, sep='\t', header=None, names=['chrom', 'region', 'ptg', 'pos', 'id', 'value', 'cov'], usecols=[0, 3, 6])
hp2_df = pd.read_csv(args.file_hp2, sep='\t', header=None, names=['chrom', 'region', 'ptg', 'pos', 'id', 'value', 'cov'], usecols=[0, 3, 6])
...
# 读取同源区域文件
hom_region_file = 'hom.region.txt'
hom_regions = pd.read_csv(hom_region_file, sep='\t', header=None, names=['start', 'end'])

# 去除merged_df中位于hom.region.txt范围内的数据
for _, region in hom_regions.iterrows():
    merged_df = merged_df[~((merged_df['pos'] >= region['start']) & (merged_df['pos'] <= region['end']))]
